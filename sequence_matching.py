...
def get_next_item(it: Iterator, passage_dict: Dict) -> Tuple[Tuple[List[Dict], List[Dict]], str]:
    ...
    :param Passage_dict: passage dict with a key as passage_id and value as passage
    :return: A tuple of ((tokenized_text, data), file_name)
    """
    while True:
        try:
...
def lcss(ref_text: List[Dict], student_input: List[Dict], prob: float = 0.1, stemmer: nltk.stem.api = None) -> \
        List[Tuple[Dict, str]]:
    """
    A function takes two lists of words in sequence as input and return using The Longest Common Subsequence algorithm
    ...
    :param stemmer: A stemmer object from nltk.Stems
    :param prob: A float number that is the threshold of confidence

    :return: A list tuple of (dict, status) where dict is a dict with key contained as
    tString and tConfidence
    """
    stemming = stemmer is not None
    lengths = [[0 for _ in range(len(student_input) + 1)] for _ in range(len(ref_text) + 1)]
...
def get_string_with_status(lcs_matching_return: List[Tuple[str, int, str]]) -> str:
    """
    A function that input a list of tuple of (dict, tuple) the dict must contain key as tString and tConfidence
    :param lcs_matching_return: a list tuple of (dict, tuple) the dict must contain key as tString and tConfidence
    :return: a string of word with status
    """
    # common_tuple = [(word, confidence, status) for word, confidence, status in lcs_matching_return]
    return ' '.join(
...
def save_match_result(file_list: Iterable[str], passage_dict: Dict[int, str], save_prefix: Path) -> None:
    """
    iterate over all files in file_list and save the result by file name under a new folder Match with extension
    :param save_prefix: prefix to the save directory
    :param file_list: an iterable contain a path to file
    ...
    for file_path in file_list:
        try:
            ...
            with open(save_prefix / f'{file_name}.txt', 'w') as f:
...
def save_result_csv(file_list: Iterable[str], passage_dict: Dict[int, str], save_prefix: Path) -> None:
    """
    Iterate over all files and get a result of Longest Common Subsequence
    and save the result by csv table with index as file name
    :param save_prefix:
    :param file_list: Iterable contains a path to file
    :param passage_dict: a dict with key as passage_id and value as passage
    """
    result = []
    for file_path in file_list:
    ...
    df.to_csv(save_prefix / 'result.csv.gz', index=False, compression='gzip')
